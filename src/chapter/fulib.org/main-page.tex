...
``What's New'' ermöglicht das Einsehen des Versionsverlaufs der Webseite und verwendeter Fulib-Bibliotheken und -Tools.
...
Auch der Schalter zum Wechseln in den Nachtmodus ist stets im Footer sichtbar.
...
Manuell lässt es sich dann weiterhin durch Klick oder Verwenden der Tastenkürzel (\code{Strg/Cmd-S}, \code{Strg/Cmd-Enter}) auslösen.
...
Im Projektarchiv enthalten ist eine voreingestellte \code{.gitignore}-Datei, um dessen Einrichtung zu erleichtern.
...
Dabei wurden zunächst die Webseite und die Scenarios in der Vorlesung erklärt.
...
Es erlaubt das Modellieren von Benutzeroberflächen als Objektstrukturen,
aus denen HTML-Seiten sowohl statisch als auch in Form von Klickprototypen generiert werden.
...
Da die Fehlermeldung des Scenario-Compilers IDs wie \code{[variable.redeclaration]} enthalten, kann in dem Datensatz leicht danach gesucht werden.
