...
Als sprachliches Mittel kommen Diagramm-Sätze zum Einsatz, welche die Markdown-Syntax für Bilder (\mdcode{![Beschreibung](dateiname.png)}) verwenden und daher keine englischen Sätze im eigentlichen Sinne sind.
...
Das Beispiel kann demnach wie folgt erweitert werden:

\begin{codeblock}
    # Example
    We call init.
    (1) Init creates a Student with name Alice.
    (2) Alice has 10 motivation.
    (3) We expect that Alice is 'Alice'.
\end{codeblock}
...
Grund dafür ist, dass das Subjekt in \code{Has}- sowie \code{Is}-Sätzen eine andere syntaktische Bedeutung hat.
%
Einige Sätze wie \code{Expect} oder Diagramme sind nur in Tests erlaubt.
Dadurch gehört der Satz hinter \code{(3)} wieder zum Rumpf der Testmethode.
