...
\code{We call produce on factory.}\\
\code{Produce creates a product.}\\
\code{Produce answers with the product.}

Diese Beispiel verwendet den \code{create}-Satz, der einem \code{There}-Satz entspricht, aber durch die Präsenz eines Subjekts in Methoden Anwendung findet.
...
\jcode{Product product = factory.produce();}
...
\code{We call produce on factory.}\\
\code{We add the answer to payload of transporter.}

\jcode{Product result1 = factory.produce();}\\
\jcode{transporter.withPayload(result1);}
...
Diese definiert eine Operation \code{getOrCreate}, die entweder gegeben eine ID entweder einen existierenden Benutzer mit dieser ID findet,
or einen neuen anlegt und speichert.
Listing~\ref{lst:UserRegistry.md} zeigt das entsprechende Scenario zeigt.
...
In Listing~\ref{lst:UserRegistry_getOrCreate.java} ist dies an \code{(1)} und \code{(2)} deutlich.
