...
Bei der Analyse werden außerdem die Fehler- und Warnmeldungen erzeugt, die die Ausgabe des Compilers bilden.
...
sie sind jedoch in Anhang~\ref{sec:gts-definitions} zu sehen.
...
Des Weiteren ist es meist vorteilhaft, dann AST vor der Auflösung zu vereinfachen.
Bildet man beispielsweise eine Art von AST-Knoten immer auf eine andere ab,
müssen folgende Schritte nicht mehr für diese erste Knotenart implementiert werden,
...
Diese Umformung erhält die Semantik, dass ein Object angelegt und einer Variable zugewiesen wird und daraufhin ein Attribute gesetzt wird.
...
Dafür muss im \code{IsSentence}-Knoten der \code{type} der \code{CreationExpr} aufgelöst werden.

Der Compiler verwendet dafür einen Scope, der aus mehreren Ebenen besteht die ihrerseits Deklarationen bereitstellen können.
...
Die Scope-Hierarchie ist in Fall der \code{CreationExpr} von außen nach innen wie folgt:
...
die Scenario-Datei für Hilfsmethoden,
die Gruppe für angelegte Klassen und der globale Scope für extern importierte Klassen zuständig.
...
Der \code{HasSentence} fügt der Scope-Hierarchie eine weitere Ebene hinzu, die den Namen \code{alice} \code{versteckt}.
...
Die Auflösung von \code{UnresolvedName(value: "name")} wird besonders gehandhabt.
Hier erfolgt nicht wie üblich eine Suche im aktuellen Scope,
sondern im Typ des \code{object}s des \code{HasSentence}.
Dies ist der Klassentyp \code{Student}, der beim Auflösen des \code{IsSentence} der Variable \code{var1} inferiert wurde.
Nun wird in diesem nach einem Attribute oder einer Assoziation namens \code{name} gesucht.
Wäre eines davon bereits vorhanden, würden der Compiler diverse Überprüfungen durchführen,
ob diese mit dem Wertetyp \code{String} des zuvor erzeugten \code{StringLiteral} kompatibel sind.
...
Listing~\ref{lst:FinalAST.txt} zeigt den vollständigen AST\@.
