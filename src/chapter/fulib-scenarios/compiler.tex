...
Mit diesem können Grammatiken in einem \ac{ebnf}-ähnlichen Format spezifiziert werden.
...
In Listing~\ref{lst:ScenarioLexer.g4} ist ein Ausschnitt der ANTLR-Grammatik zu sehen, welche diesen definiert.
...
Auf der obersten Ebene werden Klassen inkl.\ Package-Name definiert, zum Beispiel \code{org.fulib.scenarios.ast.Node}.
...
Diese werden entweder im selben Package (zum Beispiel \code{org.fulib.scenarios.ast.ScenarioFile}) oder in einem Unterpackage (\code{org.fulib.scenarios.ast.decl.Name}) abgelegt.
...
Weiterführendes Ziel ist es, den im vorherigen Abschnitt angelegten \ac{ast}, der in Listing~\ref{lst:CompilationExampleAST.txt} gezeigt wurde, weiter zu verarbeiten.
...
\codelisting{java}{chapter/fulib-scenarios/trees}{Preprocessed\ac{ast}.txt}{\ac{ast} nach Umformung von \code{ThereSentence} zu \code{IsSentence} + \code{HasSentence}}

\codelisting{java}{chapter/fulib-scenarios/definitions}{FulibScenarios2.gts}{Neue \ac{ast}-Klassendefinitionen für Listing~\ref{lst:Preprocessed\ac{ast}.txt}}

Im nächsten Schritt werden die neu erzeugten \code{Sentence}-Knoten aufgelöst.
Meist ist an dem Wort \code{Unresolved} im Klassennamen zu erkennen, dass ein \ac{ast}-Knoten aufgelöst werden muss.
...
\codelisting{java}{chapter/fulib-scenarios/trees}{Final\ac{ast}.txt}{\ac{ast} nach Analyse und Transformation}
