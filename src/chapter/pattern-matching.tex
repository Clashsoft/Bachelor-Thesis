...
Das Objektdiagramm in Abbildung~\ref{fig:tables-object-diagram} zeigt, wie das Datenmodell von Tabellen aufgebaut ist und wie sich verschiedene Tabellen-Instanzen die Daten teilen.
...
Dies lässt sich wie folgt darstellen\footnote{FulibTables ermöglicht das Rendern von Tabellen als Markdown- und HTML-Tabellen, die visuell den hier gezeigten Beispielen entsprechen}:
...
Dies ergibt die Anzahl der Units, ohne dass der Name des Attributs angegeben werden musste.
...
Die Werte befinden sich in einer \code{Map}, die einen Spaltennamen zu dem Wert in der Zeile dieser Spalte zuordnet.
...
Alternativ kann er auch direkt mit dem \code{builder} angelegt werden kann.
...
Das folgende Beispiel zeigt den einfachsten möglichen Match-Satz, der nach genau einem beliebigen Objekt sucht und es an den Namen \code{g} bindet.
...
Ohne weitere Einschränkung kann mit diesem Beispiel kein erfolgreicher Match erzielt werden,
...
Wenn mehr als ein Objekt wie z.B.\ \code{game} und \code{Alice} deklariert wurden, sind sowohl \code{g=game, p1=Alice} als auch \code{g=Alice, p1=game} mögliche Zuweisungen des Patterns.
...
diesen unter der Liste von Pattern-Objekten zu platzieren.
...
\begin{quote}
    ...
    \mdcode{![Game Model Object Diagram](https://i.imgur.com/ZzJRVW8.png)}
\end{quote}

Aufgrund der umfassenderen Task-Beschreibungen ist dieser relativ kurz gehalten.
