...
Durch Auflisten aller Attribute und anschließendes Filtern konnte eine Spalte für die Anzahl der Units erstellt werden, ohne dass der Name des Attributes angegeben werden musste.
...
Maßgeblich für die Suche nach dem Gewinner ist die Bedingung, dass die Anzahl der Units größer als die vom Spiel vorgegebene Mindestanzahl ist.
...
Zu beachten ist jedoch die darauffolgende Angabe der verwendenden Pattern-Objekte.
...
Dies erfolgt durch Anwenden der \code{filter}-, \code{expand} / \code{expandAll} und \code{filterRows}-Operationen auf der internen Tabelle.
...
Ein Match-Satz ist erkennbar an dem Schlüsselwort \code{match} bzw.\ \code{matches}, dem ein Subjekt voransteht.
...
% Mehrere Root-POs
Beide Schreibweise bewirken, dass sowohl nach \code{g} als auch nach \code{p1} unter allen Root-Objekten gesucht wird.
...
Durch Wiedereinführen des Attribut-Constraints des Spielernamens kann das Problem behoben werden.
